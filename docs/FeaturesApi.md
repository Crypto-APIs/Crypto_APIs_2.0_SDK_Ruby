...
> <GenerateAddressR> generate_address(blockchain, network, opts)
...
opts = {
  context: 'context_example', # String | In batch situations the user can use the context to correlate responses with requests. This property is present regardless of whether the response was successful or returned as an error. `context` is specified by the user.
  generate_address_rb: CryptoApis::GenerateAddressRB.new({data: CryptoApis::GenerateAddressRBData.new({item: 3.56})}) # GenerateAddressRB | 
}

begin
  # Generate Address
  result = api_instance.generate_address(blockchain, network, opts)
  p result
rescue CryptoApis::ApiError => e
  puts "Error when calling FeaturesApi->generate_address: #{e}"
end
```

#### Using the generate_address_with_http_info variant

This returns an Array which contains the response data, status code and headers.

> <Array(<GenerateAddressR>, Integer, Hash)> generate_address_with_http_info(blockchain, network, opts)

```ruby
begin
  # Generate Address
  data, status_code, headers = api_instance.generate_address_with_http_info(blockchain, network, opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <GenerateAddressR>
rescue CryptoApis::ApiError => e
  puts "Error when calling FeaturesApi->generate_address_with_http_info: #{e}"
end
```
