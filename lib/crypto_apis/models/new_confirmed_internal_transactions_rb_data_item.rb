  ...
  class NewConfirmedInternalTransactionsRBDataItem
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CryptoApis::NewConfirmedInternalTransactionsRBDataItem` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CryptoApis::NewConfirmedInternalTransactionsRBDataItem`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @allow_duplicates.nil?
        invalid_properties.push('invalid value for "allow_duplicates", allow_duplicates cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @allow_duplicates.nil?
    ...
    def hash
      [address, allow_duplicates, callback_secret_key, callback_url, receive_callback_on].hash
    end
