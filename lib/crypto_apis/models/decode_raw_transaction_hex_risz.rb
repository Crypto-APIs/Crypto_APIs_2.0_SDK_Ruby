  ...
  class DecodeRawTransactionHexRISZ
    ...
    def self.openapi_types
      {
        ...
        :'vin' => :'Array<DecodeRawTransactionHexRISZVinInner>',
        :'vout' => :'Array<DecodeRawTransactionHexRISZVoutInner>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CryptoApis::DecodeRawTransactionHexRISZ` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CryptoApis::DecodeRawTransactionHexRISZ`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'saplinged')
        self.saplinged = attributes[:'saplinged']
      end
    ...
    def list_invalid_properties
      ...
      if @saplinged.nil?
        invalid_properties.push('invalid value for "saplinged", saplinged cannot be nil.')
      end
    ...
    def hash
      [expiry_height, locktime, overwintered, saplinged, transaction_hash, value_balance, version, version_group_id, vin, vout].hash
    end
